...
time_len = len(tmp_brightened)
active_brightened = np.zeros((nmax,time_len))
tmp_darken[nnt_delay:] = active_darken[:(nnn-nnt_delay)] 
active_darken = (tmp_darken/0.3)*0.90

# artificial time delay
for iraw in range(0,4*4+1):
    for icol in range(3,2*4+1):
        iloc = 4*3*iraw + icol
        tmv  = 5.0+ iraw * 0.8
        nnn = len(tmp_brightened)
        nnt_delay = int(tmv/dt)  
        mbrightened = np.zeros_like(tmp_brightened) 
        mbrightened[nnt_delay:] = tmp_brightened[:(nnn-nnt_delay)] 
        active_brightened[iloc,:] = 1.00 * mbrightened
...
#tmp_brightened[nnt_delay:] = active_brightened[:(nnn-nnt_delay)] 
#active_brightened = 3.0 * tmp_brightened
...
tt_bright = 17*6  #Net_settings['nmax'] - tt_dark             # remaining Hypercolumns would receive long stationary bar
...
active_location = [0]
for i in range(6*6):#Net_settings['nmax']):
    idloc = id_cue[0][i]
    active_location.append(idloc)
for i in range(17*6):#Net_settings['nmax']):
    idloc = id_stm[0][i]
    active_location.append(idloc)
                       
active_type = ['b']
for i in range(1,23*6):#1,Net_settings['nmax']):
...
#                if((iloc >=0)&(iloc<18)):
                if(stm_type=='darken'):
                    particular_stimulus = np.zeros_like(stimuli_collection['darken'])
                    print('before:',np.shape(particular_stimulus))
                else:
                    if(stm_type=='darken'):
                        print('External Input Mistake!')
                    mvstimulus = active_brightened
                    particular_stimulus = mvstimulus[iloc,:]
                    plt.plot(particular_stimulus)
                    print('after:',np.shape(particular_stimulus))
                    plt.pause(0.5)
