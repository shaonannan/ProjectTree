...
Net_settings = {'hyp_num': 15,
                ...
                'dt':0.1,
                'dv':1e-3}
# >>> Connectivity strengths are normalized by number of moments within each hyper-column as well as orientation clusters >>>
...
print('Number of hyper-column: ',Net_settings['hyp_num'])
...
"""
# >>> Generate orientation map, 'xn' represents number of rows/cols within one hyper-column, odd number(2k+1) the center one is ...
# >>> even one(2k) left and right
...
# >>> Here, things went more complex, conditions were divided into: on, xhyp>1\xhyp<=1, if xhyp<=1, code above is enough, or further process is needed
if(Net_settings['xhyp'] > 1):
    ...
    # ori[start:start+Net_settings['xn']*Net_settings['yn']] = np.squeeze(np.reshape(ori_2d[Net_settings['xn']*1:Net_settings['xn']*2,:],(1,Net_settings['xn']**2)))
    
    # then here are two templates for 2*k / 2*k+1
    # for 2*k
    # >>> Two templates, one for even hyper-column(row), the other for odd hyper-column(row) >>>
    ...
    # >>> Only have effects on x(long) axis, means being constrainted in the first row 
...
# >>> Deal with map-structure in each column(cols)
...
'''
# >>>>>>>>>>>>>>>>>>>>>>>>>>>> Simplest Version, one two three.... >>>>>>>>>>>>>>>>>>>>>>>


# >>> Create narray to record: Hypercol[0->x-index for hyp,1->y-index for hyp,2->total-index for hyp].
# >>> Phase-map (random-chosen 'e'/'i',flatten to one-dimension)
# >>> Orientation-map(flatten to one-dimension)
# >>> Index->location[0->index for hyp,1->x-index,2->y-index]
...
# >>>>>>>>>>>>>>>>>>>>>>>>>> Principle Code >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# >>> Backup and Principle to define functions like: def preprocess_for_distance_period: and def preprocess_for_distance
...
# >>> Functins which support the generation of visual stimuli >>>
def utilhvsvec(x):
    f = np.maximum(x,0)
    return f

# >>> Alpha-function to generate temporal kernel >>>
...
#        ''' if use these uncorrect code, Ull still get the correct result,
#        'Cause the uncorrect results would be replace in the last iteration,
#        and then, get the correct results using the final computing value'''
...
#        t_darken     = utilhvsvec(t_darken)
    ...
    # >>> This code is subsequent to calculation of t_resp, only when we already have t_resp for on and off visual pathway, could we calculate accurate responses to brightened and darken visual stimuli >>>
    ...
    # >>> Here, using HEAVISIDE(vector)-function to ensure reliability of our input data >>>
    t_brightened = utilhvsvec(t_brightened)
    t_darken     = utilhvsvec(t_darken)
    return t_resp,t_brightened,t_darken

# >>> Generate one-dimension Function maps, like orientation, phase, index in hypercolumn and so on >>>
...
# >>> This is used in Master-equation method, not in Moment-method >>>
...
# because we all use darken stimulus, active_brightened <-- active_darken
active_brightened = active_darken


# >>> Visual stimuli assignment and design. Here we use some classical characters to describe and classify each population, like, receiving darken or brightened stimulus, location.
# >>> A trick, here we marked the position receive 'cue' stimulus >>>
stimuli_collection = {}
tt_delay = 10.0
...
# >>> Artificial time-lag(delay) >>>

# >>>>>>>>>>>>>>> Time settings for 'SLOW MOVING BLOCK' stimulus >>>>>>
# >>> ......
# >>>>>>>>>>>>>> Time settings for 'TRADITIONAL LINE-MOTION ILLUSION' stimulus >>>> 
t_delay_brightened = 68.0
tmp_brightened = np.zeros_like(active_brightened) 
nnn = len(active_brightened)
...
tt_dark   = 6*6
tt_bright = 8*6
# >>> Indeed, locations, receive brightened stimulus, contain (tt_dark + tt_bright), 'cue' region which receive darken stimulus at first, also receive brightened stimulus later. >>>
...
def input_signal_stream(active_location, active_type, active_resp,t_properties,Net_settings):
    # >>>>>>>>>>>>> Setting & Showing(Testing) 'CUE' stimulus >>>>>>>>>>>>>>
    ...
    # >>>>>>>>>>>>>>> Create base-visual stimuli(Background Firing-rate) >>>>>>>>>>>
        ...
        for iactive in range(len(active_type)):
            ...
            if(celltype == 'b'):
                if(stm_cue[iloc,0] ==1 ):
                    ...
                    particular_stimulus = 0.0*stimuli_collection['brightened']+stimuli_collection['darken']
                else:
                    # >>> Indeed, regions receive darken stimulus should be restricted in 'CUE' region, at no circumstance should darken regions occur outside 'CUE' region >>>
