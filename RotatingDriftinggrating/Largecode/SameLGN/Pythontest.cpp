...
int main(int argc, char *argv[]) {
  ...
  // LR spreading
  double llee2 = 1.50 * 1.50 / 1.0;
  double llie2 = 1.50 * 1.50 / 1.0;
  double dx = 0.5 / ni;
  double dy = 0.5 / nj;
  double dx2 = dx * dx;
  double dy2 = dy * dy;
  ...
  pconn = 0.45;     // sparseness ? DONOT NEED TO BE SPARSE
  pconnlr = 0.125;
  ...
  */
  // double seemax = 0.1678 / 1.00 ;  // peejjkk = 0.5*other
  // double seimax = 0.1481 / 1.00 ;
  // double siemax = 0.1583 / 1.00 ;
  // double siimax = 0.028 / 1.00 ;  // 1.261/1.0;

 /* >>>>> Recurrent Dominant >>> */ 
  double seemax = 0.678 / 1.00 ;  // peejjkk = 0.5*other
  double seimax = 0.481 / 1.00 ;
  ...
  lee = 2.64 * 1.00/ tau_e * 4.0 / 3.0 / pconnlr/1.0*1.0 ;
  lie = 6.42 * 1.00 / tau_e * 4.0 / 3.0 / pconnlr/1.0*1.0;

  lief = 1.563*(0.250+0.250)*0.5/ tau_e * 4.0/3.0  / pconnlr;
  leef = 1.507*(0.250+0.250)*0.5/ tau_e * 4.0/3.0  / pconnlr;
  ...
  // gaussk(16.00,lr_ee,aa,ni,nj,llee2,dx2,dy2,fglee);
  // gaussk(16.00,lr_ie,aa,ni,nj,llie2,dx2,dy2,fglie);
  gaussk(1.250, lr_ee, aa, ni, nj, llee2, dx2, dy2, fglee, 1.5);
  gaussk(1.250, lr_ie, aa, ni, nj, llie2, dx2, dy2, fglie, 1.5);
  ...
  // voltage
  if ((Vmem = fopen("vmemd.txt", "w")) == NULL) {
    printf("cannot open the output file exactly!\n");
    exit(0);
  }
  if ((Vslave = fopen("vslaved.txt", "w")) == NULL) {
    printf("cannot open the output file exactly!\n");
    exit(0);
  }
  if ((Gexc = fopen("gexcd.txt", "w")) == NULL) {
    printf("cannot open the output file exactly!\n");
    exit(0);
  }
  if ((Ginh = fopen("ginhd.txt", "w")) == NULL) {
    printf("cannot open the output file exactly!\n");
    exit(0);
  }
  if ((Gtot = fopen("gtotd.txt", "w")) == NULL) {
    printf("cannot open the output file exactly!\n");
    exit(0);
  }

  // firing rate
  if ((FR = fopen("frd.txt", "w")) == NULL) {
    printf("cannot open the output file exactly!\n");
    exit(0);
  }
  ...
  for (int iii = 0; iii < ntotal; iii++) {
    ...
    if (((iii % nstep0) == 0) && (t > 2 * dt)) {  //(t > 2.0
      ...
      for (int i = 0; i < nmax; i++) {
        ...
        cond[i][ncycle]    = gl[i]; // -beta1[i];// + cond[i][ncycle];
...
void visual(double frtlgn0, double t, unsigned iseed, int StimuliType) {
  ...
  */
  ...
  double sd = 0.19;//double sd = 0.23;
  double dt = 0.0001;
  int itt = (floor)(t/dt/10.0);
  printf("Time bin  %d !\n", itt);
  for(int i = 0;i<nmax;i++){
    gl[i] = pregl[i][itt]*sd * 9.8 +sd *heaviside( (rand() / (RAND_MAX + 1.0) - 0.5) * 5.0 * frtlgn0 );
  }
  return;
}
...
void save_params() { // @TODO: 修改参数表，与声明一致
  ...
  const static char *save_dir = "/disk1/home/cosmo/Documents/PytorchMoments/SYX_PDEs/LARGE_CODE_ROTATE/parameters/"; // @TODO: 修改存放目录
  // File name
