...
class RecurrentPopulation(object):
    ...
    def __init__(self,tau_m = 20.0,dt = 0.1,v_min = -1.0,v_max = 1.0,dv = 1e-3,record = True,
        ...
        self.Pinit = 0.0 # P to do normalization
    ...
    def initialize_prob(self):
        ...
        var1   = np.power((5/3.0/250.0),2)  # M2-M1^2
        ...
        # Here, calculate and record P-init
        GaussP0 = np.exp(-np.square(vedges-0.0)/var1/2.0)
        self.Pinit = (h*np.sum(np.squeeze(GaussP0)))
        ...
        self.La0 = La0
        self.fin = fin
        # save initial parameters,to ensure that, these initial(reset-)parameters
        # could be reused at sigular-value
        self.source    = source 
        self.initLa0   = La0
    ...
    def update_total_fpmu_dict(self):
        ...
        # Gaussian Approximation or Not
        if self.total_fp_vslave < 1.0:
            self.USUALorGauss = 0
        else:
            self.USUALorGauss = 1
    ...
    def update_fp_moment4(self):
        vT = 1.0 # use this to do judgement
        v1 = self.v1.copy()
        v2 = self.v2.copy()
        v3 = self.v3.copy()
        v4 = self.v4.copy()
    ...
    def update_fp_moment4_Gauss(self):
        vT = 1.0 # use this to do judgement
        v1 = self.v1.copy()
        v2 = self.v2.copy()
        v3 = self.v3.copy()
...
#        v4n = v4 + dtgL*(-fr/gL - 4.0*(v4-vs*v3-1.5*ds*v2))
        ...
        v1n = v1 + dtgL*(- (v1-vs)) # -fr/gL - (v1-vs))
        v2n = v2 + dtgL*(- 2.0*(v2-vs*v1-0.5*ds)) # -fr/gL - 2.0*(v2-vs*v1-0.5*ds))
        v3n = v3 + dtgL*(- 3.0*(v3-vs*v2-ds*v1)) # -fr/gL - 3.0*(v3-vs*v2-ds*v1))
        v4n = v4 + dtgL*(- 4.0*(v4-vs*v3-1.5*ds*v2)) # -fr/gL - 4.0*(v4-vs*v3-1.5*ds*v2))
        ...
        #print('inter moment: v1 %.5f %.5f'%(self.v1,self.v2))
    def update_GauApprox_rhoEQ(self):
        gL   = 1.0/self.tau_m
        ...
        M2 = self.v2.copy()
        M2 = M2 - M1**2
        Vs,D,V = M1.copy(), M2.copy(), vedges.copy()
        Rv = np.copy(V)
        (vT,vR) = (1.0,0.0)
        ...
        try:
            Rv[indp[0][:]] = np.exp(-np.square(V[indp[0][:]]-Vs)/2.0/D)#np.exp(-(V[indp[0][:]]-Vs)**2/D)
            
            if(indp[0][0]>1):
                Rv[0:indp[0][0]] = np.exp(-np.square(V[0:indp[0][0]]-Vs)/2.0/D)
    ...
    def update(self):
    	if self.USUALorMFE:
    		...
    		if self.USUALorGauss:
    			print('GaussNot! Larger than vT: ',self.total_fp_vslave)
    			self.update_fp_moment4()
    			self.update_trad_rhoEQ()
    		else:
    			print('Gauss! Less than vT: ',self.total_fp_vslave)
    			self.update_fp_moment4_Gauss()
    			self.update_GauApprox_rhoEQ()
    	else:
    		self.update_total_fpsig_dict()
    		self.update_total_fpmu_dict()
    		self.update_rhoEQ()
    	if self.USUALorGauss < 1:   
    		vs   = self.total_fp_vslave
    		ds   = self.total_fp_sigv
    		...
    		rhoEQ  = self.rhoEQ
    		rhov   = rhoEQ.copy()
    		firing_rate  = gL*rhov[-1]*(-1.0+vs)-gL*ds/2.0*rhov[-1]*(-(1.0-M1)/M2) # 

    		if firing_rate <0:
    			print('Gauss! But uncorrect fr: ',firing_rate)
    			self.update_fp_moment4()
    			self.update_trad_rhoEQ()
    			self.USUALorGauss = 1
    	...
    	if self.USUALorGauss<1:
    		...
    		if self.USUALorMFE:
    			...
    			if firing_rate <0:
    				self.USUALorGauss = 1
    			if np.isnan(firing_rate)|np.isnan(np.sum(rhov))|np.isinf(firing_rate)|np.isinf(np.sum(rhov))|np.isneginf(firing_rate)|np.isneginf(np.sum(rhov)):
    				print('time: ',self.simulation.t,'fr: ',firing_rate,' ds: ', ds,' sumrho: ', sum_rhoEQ)
    				self.USUALorGauss = 1
    			if self.USUALorGauss < 1:
    				self.rhov = rhov
    				self.rhoEQ = rhoEQ
    	...
    	if self.USUALorGauss>0:
    		...
    		if self.USUALorMFE:
    			...
    			if np.isnan(firing_rate)|np.isnan(np.sum(rhov))|np.isinf(firing_rate)|np.isinf(np.sum(rhov))|np.isneginf(firing_rate)|np.isneginf(np.sum(rhov)):
    				...
    				firing_rate = 0.0
    				# when encounter the singular point, plz reset all state parameters and variables
    				rhov        = self.source
    				...
    				self.hnmda = 0.0
    				# ME HYPERPARAMETERS
    				self.La1 = self.initLa0 # np.zeros_like(self.La1)
    				self.La0 = self.initLa0 #np.zeros_like(self.La0)
    				# also, correlated parameters and variables
